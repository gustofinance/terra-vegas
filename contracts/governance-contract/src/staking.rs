...
use terra_vegas::distribution::ExecuteMsg;
use terra_vegas::gov::{PollStatus, StakerResponse};
...
pub fn withdraw_voting_tokens(
    ...
    if let Some(mut token_manager) = bank_read(deps.storage).may_load(key)? {
        ...
        if locked_share + withdraw_share > user_share {
            ...
            .add_message(unbond_tokens_distribution_contract(
                deps,
...
fn unbond_tokens_distribution_contract(
    deps: DepsMut,
    distribution_contract: &CanonicalAddr,
    recipient: &CanonicalAddr,
    amount: u128,
) -> Result<CosmosMsg, ContractError> {
    let distribution_human = deps.api.addr_humanize(distribution_contract)?.to_string();
    let recipient_human = deps.api.addr_humanize(recipient)?;

    let msg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: distribution_human,
        msg: to_binary(&ExecuteMsg::UnbondStake {
            address: recipient_human,
            amount: Uint128::from(amount),
        })?,
        funds: vec![],
    });
